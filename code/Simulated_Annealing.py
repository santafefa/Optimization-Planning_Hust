...
class Simulated_Annealing:
            ...
            if load[v_idx] > 0 and load[v_idx] < self.lower_bound[v_idx]:
                for ord_idx, veh_idx in enumerate(assignments):
                    if veh_idx == v_idx:
                        ...
                        curr_cost -= self.cost[ord_idx]
                load[v_idx] = 0
        return assignments, load, curr_cost
    def add_unassigned(self, assignments, load):
        ...
        return None
    def neighboor_remove(self, assignments, load):
        ...
        if self.check_validation(new_load[remove_v], remove_v):
            new_assignments = assignments[:]
            new_assignments[remove_ord] = -1
            return new_assignments, new_load
        return None
        ...
        for new_v in random.sample(range(self.vehicles_num), self.vehicles_num):
            if new_v != remove_v:
                if load[new_v] + self.quantity[remove_ord] <= self.upper_bound[new_v]:
                    ...
                    new_load[remove_v] -= self.quantity[remove_ord]
                    if self.check_validation(new_load[remove_v], remove_v) and self.check_validation(new_load[new_v],
                                                                                                     new_v):
    ...
    def generate_neighboor_swap(self, assignments, load):
        ...
        if new_load_v1 <= self.upper_bound[v1] and new_load_v2 <= self.upper_bound[v2]:
            if self.check_validation(new_load_v1, v1) and self.check_validation(new_load_v2, v2):
                ...
                return new_assignments, new_load
        return None
    def solve(self, initial_temperature=2000, cooling_state=0.995, time_limit=0.8):
        self.read_input()
        start_time = time.time()
        best_assignment = [-1] * self.orders_num
        objective_value = -1
        cur_assign,cur_load, cur_obj=self.greedy()
        ...
        while True:
            if time.time() - start_time > time_limit:
                break
            new_assignments, new_load = None, None
            choice = random.random()
            if choice < 0.3:
                res = self.neighboor_remove(cur_assign, cur_load)
            elif choice < 0.6:
            ...
            if res is not None:
                new_assignments, new_load = res
                new_obj = sum(self.cost[i] for i, v in enumerate(new_assignments) if v != -1)
                if new_obj > cur_obj:
                    accept_rate=1.0
                else:
                    accept_rate = math.exp((new_obj - cur_obj) / temp)

                if random.random() < accept_rate:
                    cur_assign = new_assignments
                    cur_load = new_load
                    cur_obj = new_obj
                    if cur_obj > objective_value:
                        best_assignment = cur_assign[:]
                        objective_value = cur_obj
            temp *= cooling_state
        # Final validation of the best assignment
        final_load= [0] * self.vehicles_num
        for i, v in enumerate(best_assignment):
            if v != -1:
                final_load[v] += self.quantity[i]
        final_assign = best_assignment[:]
        for v_idx in range(self.vehicles_num):
            if not self.check_validation(final_load[v_idx], v_idx):
                for i, v in enumerate(final_assign):
                    if v == v_idx:
                        final_assign[i] = -1
        fin_obj = sum(self.cost[i] for i, v in enumerate(final_assign) if v != -1)
        return final_assign, fin_obj
